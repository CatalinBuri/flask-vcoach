...
# =========================
# HELPER WRAPPER ENDPOINTS (PENTRU NOUA INTERFAȚĂ FRONTEND / JS)
# =========================
...
def api_reformulate_cv_alias():
    if request.method == "OPTIONS":
        return api_response(code=200)
    return reformulate_cv_for_job_boards()


@app.route("/api/analyze-cv", methods=["POST", "OPTIONS"])
@cross_origin()
def api_analyze_cv_alias():
    if request.method == "OPTIONS":
        return api_response(code=200)
    return analyze_cv()


@app.route("/api/generate-queries", methods=["POST", "OPTIONS"])
@cross_origin()
def api_generate_queries_alias():
    if request.method == "OPTIONS":
        return api_response(code=200)
    return generate_job_queries()


@app.route("/api/linkedin-optimize", methods=["POST", "OPTIONS"])
@cross_origin()
def api_linkedin_optimize_alias():
    if request.method == "OPTIONS":
        return api_response(code=200)
...
@cross_origin()
def api_evaluate_answer_alias():
    if request.method == "OPTIONS":
        return api_response(code=200)
    return evaluate_answer()


@app.route("/api/coach-star", methods=["POST", "OPTIONS"])
@cross_origin()
def api_coach_star_alias():
    if request.method == "OPTIONS":
        return api_response(code=200)
    return coach_next()


@app.route("/api/coach-questions", methods=["POST", "OPTIONS"])
@cross_origin()
def api_coach_questions_alias():
    if request.method == "OPTIONS":
        return api_response(code=200)
    return generate_coach_questions()
