...
MODEL_NAME = "gemini-2.5-flash"  # recomandat – rapid și bun la instrucțiuni
        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "Ești un expert LinkedIn Job Search și recruiter"
                    " profesionist. Răspunde NUMAI cu JSON valid atunci când se"
                    " solicită."
                ),
            },
          ...
          messages=[
              {
                  "role": "system",
                  "content": (
                      "Ești un recrutor profesionist cu peste 10 ani de"
                      " experiență umană, asistat de inteligență artificială"
                      " avansată. Îmbină empatia, intuiția și feedback-ul"
                      " constructiv uman cu analiza obiectivă, riguroasă și"
                      " bazată pe date a unui sistem AI. Răspunde mereu cu"
                      " maxim de profesionalism, obiectivitate și motivație"
                      " pentru dezvoltarea candidatului. Respectă EXACT"
                      " instrucțiunile: dacă ți se cere JSON, returnează NUMAI"
                      " JSON valid, fără text suplimentar, fără ```, fără"
                      " markdown. Dacă ți se cere text simplu, răspunde NUMAI"
                      " cu text curat, fluent și profesionist în română, fără"
                      " asteriscuri, bold, liste marcate sau alte elemente de"
                      " formatare."
                  ),
              },
              {"role": "user", "content": prompt},
...
- Formulează în română profesională, clară, naturală, corectă din punct de vedere gramatical și folosind cuvinte comune, fără termeni tehnici.
  ...
  if not parsed or "questions" not in parsed or len(parsed["questions"]) != 7:
    parsed = {
        "questions": [
            ...
            (
                "Povestește despre o situație dificilă pe care ai gestionat-o la"
                " locul de muncă."
            ),
            "Ce te motivează cel mai mult atunci când lucrezi într-o echipă?",
            (
                "Cum recepționezi și aplici feedback-ul primit de la colegi sau"
                " manageri?"
            ),
            (
                "Care sunt așteptările tale realiste de la următorul rol"
                " profesional?"
            ),
        ]
    }
...
def coach_generic_eval():
  ...
  if len(answer.split()) < 5:
    return api_response(payload={
        ...
        "improved_answer": (
            "Dezvoltă-ți ideile cu exemple personale pentru a primi feedback"
            " complet și o variantă optimizată."
        ),
        "nota_finala": 4,
    })
...
"""
  ...
  if not parsed or "feedback" not in parsed or "improved_answer" not in parsed:
    parsed = {
        "feedback": (
            "Răspunsul tău arată potențial și autenticitate. Pentru un impact mai"
            " mare, adaugă un exemplu concret și structurează ideile mai clar."
            " Continuă să exersezi – ești pe drumul bun!"
        ),
...
"""
  ...
  final_payload = {
      "clarity_score": (
          int(sum(clarity_scores) / len(clarity_scores))
          if clarity_scores
          else 0
      ),
      "relevance_score": (
          int(sum(relevance_scores) / len(relevance_scores))
          if relevance_scores
          else 0
      ),
      "structure_score": (
          int(sum(structure_scores) / len(structure_scores))
          if structure_scores
          else 0
      ),
      "overall_assessment": (
          "CV-ul a fost analizat și scorurile au fost calculate."
      ),
...
def analyze_cv():
  try:
    ...
    for i, (cv_chunk, job_chunk) in enumerate(
        zip_longest(cv_chunks, job_chunks, fillvalue="")
    ):
      prompt_chunk = f"""
...
"""
      ...
      if (
          not parsed_chunk
          or "compatibility_percent" not in parsed_chunk
          or "feedback_markdown" not in parsed_chunk
      ):
        parsed_chunk = {
            "compatibility_percent": 70,
            "feedback_markdown": (
                "Fragmentul CV-ului are relevanță parțială pentru cerințele"
                " acestui segment al jobului."
            ),
        }
...
"""
    ...
    final_score = (
        int(sum(chunk_scores) / len(chunk_scores)) if chunk_scores else 75
    )
...
"""
  ...
  if (
      ...
      or len(parsed.get("questions", [])) != 5
  ):
    parsed = {
        "questions": [
            (
                "Povestiți despre experiența dumneavoastră cea mai relevantă"
                " pentru acest post."
            ),
            (
                "Care considerați că sunt principalele dumneavoastră puncte"
                " forte în relație cu cerințele rolului?"
            ),
            (
                "Descrieți o situație challenging din carieră și modul în care"
                " ați gestionat-o."
            ),
            (
                "Ce vă motivează să aplicați pentru această poziție în compania"
                " noastră?"
            ),
            (
                "Cum abordați învățarea continuă și adaptarea la tehnologii"
                " noi?"
            ),
        ]
    }
    ...
    if (
      ...
      return api_response(
          payload={
              ...
              "message": (
                  "Experiența candidatului este prea nișată sau formulată într-un"
                  " mod care nu permite asocierea clară cu roluri standard de"
                  " pe platformele de joburi."
              ),
          }
      )
...
"""
  ...
  if not parsed:
    parsed = {
        ...
        "linkedin_about": (
            "Profil LinkedIn optimizat profesional pe baza experienței"
            " dumneavoastră."
        ),
    }
  return api_response(payload=parsed)
  ...
  try:
    ...
    if len(answer.split()) < 5:
      return api_response(
          payload={
              "star_answer": (
                  "Răspunsul este prea scurt pentru a fi restructurat în format"
                  " STAR."
              )
          }
      )
...
"""
  ...
  if not parsed or "nota_finala" not in parsed:
    parsed = {
        ...
        "nota_finala": 7,
        "feedback": (
            "Răspunsul este coerent, dar poate fi îmbunătățit printr-o"
            " structurare mai clară și exemple mai relevante."
        ),
    }
...
"""
  ...
  if not parsed:
    parsed = {
        "summary": (
            "Candidatul a demonstrat competențe solide și o atitudine"
            " profesionistă pe parcursul interviului. Există potențial ridicat,"
            " cu recomandări minore de îmbunătățire a structurii răspunsurilor."
        ),
        "scor_final": 8,
    }
  return api_response(payload=parsed)
...
"""
    ...
    if not parsed or not all(k in parsed for k in required_keys):
      return api_response(
          error=(
              "AI nu a putut genera un rezultat valid pentru reformularea"
              " CV-ului"
          ),
          code=503,
      )
...
# =========================
# HELPER WRAPPER ENDPOINTS (PENTRU NOUA INTERFAȚĂ JS)
# =========================
@app.route("/api/scrape-job", methods=["POST", "OPTIONS"])
@cross_origin()
def api_scrape_job_alias():
  return scrape_job()


@app.route("/api/interview", methods=["POST", "OPTIONS"])
@cross_origin()
def api_interview_alias():
  if request.method == "OPTIONS":
    return api_response(code=200)

  data = request.get_json(force=True) or {}
  cv = data.get("cv") or data.get("cv_text") or ""
  job = data.get("job_description") or data.get("job_summary") or ""

  if not cv or not job:
    return jsonify({"question": "Te rugăm să furnizezi atât CV-ul, cât și descrierea jobului."})

  prompt = f"Pe baza acestui CV:\n{cv[:2000]}\nȘi a jobului:\n{job[:2000]}\nGenerează prima întrebare directă de interviu în română."
  q = gemini_text(prompt)
  return jsonify({"question": q or "Descrieți experiența dumneavoastră principală relevantă pentru acest post."})


@app.route("/api/cv-quality", methods=["POST", "OPTIONS"])
@cross_origin()
def api_cv_quality_alias():
  if request.method == "OPTIONS":
    return api_response(code=200)
  return analyze_cv_quality()


@app.route("/api/reformulate-cv", methods=["POST", "OPTIONS"])
@cross_origin()
def api_reformulate_cv_alias():
  if request.method == "OPTIONS":
    return api_response(code=200)

  data = request.get_json(force=True) or {}
  cv = data.get("cv") or data.get("cv_text") or ""
  job = data.get("job_description") or data.get("job_text") or ""

  prompt = f"Reformulează și optimizează următorul CV pentru jobul descris:\n\nCV:\n{cv}\n\nJob:\n{job}"
  reformatted = gemini_text(prompt)
  return jsonify({"reformatted_cv": reformatted or cv})
